...
void DrawCGImageWithHQXScaling(CGContextRef context, CGImageRef sourceImage, int scaleBy)
{
	size_t width = CGImageGetWidth(sourceImage);
	size_t height = CGImageGetHeight(sourceImage);
	
	CGDataProviderRef sourceDataProviderRef = CGImageGetDataProvider(sourceImage);
	CFDataRef sourceDataRef = CGDataProviderCopyData(sourceDataProviderRef);
	
	size_t srcSize = CFDataGetLength(sourceDataRef);
    uint8_t *srcData = (uint8_t *) malloc(srcSize);
	
	CFDataGetBytes(sourceDataRef, CFRangeMake(0, CFDataGetLength(sourceDataRef)), srcData);
	
    size_t destSize = CFDataGetLength(sourceDataRef)*pow(scaleBy, 2);
    ...
    switch (scaleBy) {
		case 2:
			hq2x_32(sp, dp, (int)width, (int)height);
			break;
		case 3:
			hq3x_32(sp, dp, (int)width, (int)height);
			break;
		case 4:
		default:
			hq4x_32(sp, dp, (int)width, (int)height);
			break;
    }
	...
	CFDataRef destDataRef = CFDataCreate(kCFAllocatorDefault, destData, destSize);
	
	CGDataProviderRef destDataProviderRef = CGDataProviderCreateWithCFData(destDataRef);
	static const size_t kComponentsPerPixel = 4;
	
	size_t layerWidth = width*scaleBy;
	size_t layerHeight = height*scaleBy;
	
	CGImageRef returnImage = CGImageCreate(layerWidth, layerHeight, CGImageGetBitsPerComponent(sourceImage),
										   CGImageGetBitsPerPixel(sourceImage),
										   kComponentsPerPixel*layerWidth,
										   CGImageGetColorSpace(sourceImage),
										   CGImageGetBitmapInfo(sourceImage),
										   destDataProviderRef, NULL, false, CGImageGetRenderingIntent(sourceImage));
	
	CGContextDrawImage(context, CGRectMake(0, 0, layerWidth, layerHeight), returnImage);
		
    // Free image data
    free(srcData);
    free(destData);
	
	CGImageRelease(returnImage);
	CGDataProviderRelease(destDataProviderRef);
	CFRelease(destDataRef);
